...
6. SQLite schema v27 按 Run 分表保存 coverage、tasks、citations、claims、conflicts 和 pages，候选对与召回原因保存在 durable consistency Task 中，材料区间和项目问题结论保存在 analysis Task 中，跨模块流程保存在 flow Task 中；保存时事务原子替换，读取时重建跨表关系、Task、区间汇总、问题汇总、文件级综合来源链、模型输入审计、候选指纹、流程 Claim 归属、Page 覆盖和页面树并复核 snapshot hash；Wiki runtime 格式独立为 schema v11；runtime v10 及更早历史任务的跨模块流程明确标记为不支持，不从旧 Claim 反推流程；
...
13. 全局一致性完成后，Host 只从 analysis Task 的“主要流程”证据结论取得可用 Claim，按 Claim 数与 statement 字符预算生成 durable flow Task。Flow Agent 必须重读每条 supports Citation，并将每条候选 Claim 恰好使用一次或明确列为未解；每条流程至少两个步骤且覆盖至少两个 Coverage，不从静态依赖图推测业务顺序。无候选时保留已完成的九问审计结论，不伪造流程；
14. flow 阶段结算后，Host 按 Coverage 区域、Claim 数和 statement 字符预算生成 durable Page Task；Page Agent 只接收任务内 Claim 状态、正文与有界路径，不读取材料，也不能提交自由事实。模型只给出任务局部 slug、标题、Claim 引用与子页面关系，Host 负责验证恰好一次覆盖、生成稳定 id、聚合状态和唯一合成根；
15. `MemoryKnowledgeEngine.planWikiRun()` 提供底层持久化入口，`MemoryKnowledge.planWikiProject()` 接入 Git tree Catalog，严格 Host RPC 与设置页可以建立计划并执行下一个 analysis、file-synthesis、verification、consistency、flow 或 page Task，并可显式加载有界活动 Page 树；新 Run 只在证据 identity 未变且支持 Citation 仍匹配时复用 verified 叶级 assertion，综合 Claim、flow、Page 和 ranged Citation 仍重新生成。
16. 最新 Run 完成 Page 阶段后，`generate` 只把已验证 assertion、单一 Git source/commit 且 Coverage 哈希匹配的 Page 变成 `suggestedBy=wiki` Knowledge Card candidate；候选继续走 review/revision CAS/promote，不绕过人工审核。Wiki Page 使用当前 Page id 派生稳定 Card id，保证同一快照重复生成幂等。
17. 每个新 analysis Task 都必须提交固定九类项目问题：目的与术语、入口、实体与状态、规则与约束、主流程、异常与恢复、持久化与集成、配置与运行时、实现与测试。`evidence` 结论只能引用同一提交的非 unknown Claim，所有非 unknown Claim 至少支持一个问题；`unknown` 与 `not-applicable` 必须保存原因。Host 从 durable Task 计算 Run 汇总和完成检查，模型不能自行标记项目问题阶段完成。

跨模块 flow 纵切已把 consistency 与 Page 之间的缺口变成 durable 阶段：当前流程结果随 flow Task 保存，Run header 和浏览器只投影候选 Claim、任务、流程、步骤与未解数量，不投影 Claim 正文、路径或 Session。详细流程图、按证据缺口自动生成的多跳回查任务、flow 版本/人工修订和 EffectiveVersion 检索投影尚未实现；当前流程是证据约束的 Claim 组织结果，不宣称静态代码已证明所有运行时路径。
...
当前实现尚未完成第一阶段全部范围。各事实阶段已有[任务累计材料读取账本](wiki-material-budget.md)；下一条主线是增加按证据缺口触发的分层多跳回查与 Run 级 token 费用控制，再处理综合 Claim、flow 和 Page 的跨 Run 细粒度复用。新的 Catalog Run 只复用证据 identity 未变且 Citation 仍匹配的 verified 叶级 assertion；综合 Claim、flow、Page 和 ranged Citation 仍安全失效。UI 已投影 Coverage、Task、大文件区间、文件级综合、九问审计、全局一致性、跨模块 flow 与 Page 的有界进度，能触发 analysis、file-synthesis、verification、consistency、flow 或 page Task。详细流程图与多跳缺口回查尚未实现；现有 TypeScript AST、关系和符号 Provider 仅是可选导航与核验能力，不是 LLM Wiki 的语言边界，也不能冒充业务流程。
