...
当前实现先落在“设置 → 记忆与知识”，覆盖范围选择、Candidate Inbox、独立的个人/项目“记忆”与项目“知识”查询、FTS 搜索、单条 Recall Trace、Source inventory、Source records 状态、模块关系分类计数、Card freshness、显式确定性 Card 候选生成、结构化 Evidence Pack 查询、有界静态模块 Graph，以及 LLM Wiki Run 覆盖/盲区摘要、显式计划入口和有界活动 Page 树。浏览查询使用显式领域与范围模式：“记忆”只返回所选范围的 Memory Entry，“知识”要求 Workspace 并只返回该项目的 Knowledge Card；Agent 默认召回继续组合个人与当前项目适用记忆。未初始化 canonical store 的项目被视为尚无知识，不作为 UI 读取错误。Host 使用严格 Typert 描述符校验输入和输出，浏览器只持有 Workspace id；本机项目根、完整逐文件 records、完整关系图与原始 Session id 不跨越 RPC，Evidence Pack 只投影命中的 portable path、行号、哈希和 Source revision，关系查询固定返回最多 120 条当前 checkpoint 边。Wiki 事实任务可以在模型侧使用有界的字面路径目录和大文件区间导航；导航不读取正文，不改变材料预算，也不授予跨任务 Citation 权限。Wiki 视图固定返回最新 20 个 Run header 摘要和最多 20 条阻塞原因；Page 树仅在用户显式请求后返回最多 100 个 Page、200 条 Claim 和 400 条 portable 来源，并分别报告省略数量。本机项目根、完整 Coverage、候选对正文和 Agent Session id 均不进入浏览器。候选审核与晋升使用 revision CAS，`restricted` 正文保持 CLI-only。逐文件树状导航、知识双视图、直接编辑、版本治理、知识实体 Graph 和请求级召回时间线仍属于后续纵切。
