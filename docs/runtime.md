...
6. `memory-knowledge-wiki-agent`：使用 public DSH Agent 与 durable Session 执行或恢复一个有界 analysis、file-synthesis、verification、consistency、flow 或 page Task；事实阶段只暴露对应的 Wiki 上下文、不可变材料读取和结构化提交工具，Page 阶段只能读取有界 Claim 上下文并提交组织关系。
...
知识域的“分析进度”辅助面板显式调用 `planWikiProject()`，显示最新 20 个 Run 的 Catalog/Coverage、Task、大文件区间、文件级综合、九问审计、全局一致性、跨模块 flow、Page 计划和阻断原因的有界汇总。同一严格 RPC 每次执行或恢复至多一个 analysis、file-synthesis、verification、consistency、flow 或 page Task。全局一致性完成后，Host 只从“主要流程”问题绑定的可用 Claim 规划 flow Task；Flow Agent 重读每条 supports Citation，Host 拒绝 Claim 遗漏、重复归属、少于两步或只覆盖一个 Coverage 的伪跨模块流程。全部 flow Task 结算后才规划 Page。Run header 不发送项目根、完整 Coverage、Claim/Citation 正文、flow 正文、候选对正文或 Agent Session id。


每个 Run header 还带有 Host 计算的十项知识激活完成报告，分别检查来源目录、纳入材料覆盖、分片分析、文件级综合、声明核验、全局一致性、页面准备、实际模型输入审计、项目问题清单和跨模块业务流程。`deferred`、blocked、stale、Catalog 遗漏、全局候选召回不完整、历史 `unassessed` 综合、未结算 flow Task 都会阻止激活；`unsupported` 必需检查同样不能通过。无 flow 候选的新 Run 可以以零任务完成该检查，因为九问审计已显式记录 unknown/not-applicable；这不会变成“项目无流程”的事实。现有旧 Run 因缺少新完成标准的持久记录只作为草稿或历史资料，不会被追认为正式项目知识。
...
历史 v25 tarball 验收时已安装到实际 Web profile，profile 依赖指向当时的持久本机发布物，安装目录导出 Wiki runtime schema v9 和材料审计规则 v1。Web Host 在 `127.0.0.1:3080` 返回 HTTP 200，实际 Provider 打开私有数据库后将 SQLite `user_version` 从 24 升级到 25。迁移后的数据库仍有 0 个 Wiki Run、0 个 Page 和 0 个 EffectiveVersion，因此没有发起 Wiki 模型请求，也没有向 MiniMax 发送项目知识。该历史验收只证明当时发布物的打包、profile 更新、Host 装载、数据库迁移及空状态保持，不代表当前 v11/v27 基线，也不证明真实模型理解质量。
